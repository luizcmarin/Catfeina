...
class PoesiaRepository @Inject constructor(
    ...
    // Método de paginação atualizado e simplificado
    fun getPoesiasPaginadas(): Flow<PagingData<GetPoesiasPaginadas>> {
        return Pager(
            config = PagingConfig(
                pageSize = 20,
                enablePlaceholders = false
            ),
            pagingSourceFactory = { PoesiaPagingSource(poesiasQueries) }
        ).flow
    }

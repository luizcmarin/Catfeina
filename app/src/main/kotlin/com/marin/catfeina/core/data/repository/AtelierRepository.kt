...
class AtelierRepository @Inject constructor(
    ...
    suspend fun upsertAteliers(ateliers: List<AtelierSync>) =
        withContext(dispatchers.io) {
        queries.transaction {
            ateliers.forEach { atelier ->
                queries.upsertAtelier(
                    id = atelier.id,
                    titulo = atelier.titulo,
                    conteudo = atelier.conteudo,
                    dataAtualizacao = atelier.dataAtualizacao,
                    fixada = atelier.fixada
                )
            }
        }
    }
